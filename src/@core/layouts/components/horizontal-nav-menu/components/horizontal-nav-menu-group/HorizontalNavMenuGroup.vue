  ...
  >
    <template v-slot:activator="{ on, attrs }">
      <v-list-item
        ...
        v-bind="attrs"
        :class="[
          { 'horizontal-nav-menu-group-active': isActive },
          { 'menu-open': isMenuActive },
        ]"
    ...
    <v-list ref="refContent" elevation="8">
...
export default {
  ...
  setup(props) {
    // eslint-disable-next-line object-curly-newline
    const { isActive, isOpen, updateGroupOpen, updateIsActive } =
    ...
    const { isOutside: isMouseOutsideOfActivator } =
      useMouseInElement(refActivator);
    const { isOutside: isMouseOutsideOfContent } =
      useMouseInElement(refContent);
      ...
      invoke(async () => {
        ...
        nextTick(() => {
          setTimeout(() => {
            ...
            const refContentTop =
              refContent.value.$el.getBoundingClientRect().top;
            const refContentHeight =
              refContent.value.$el.getBoundingClientRect().height;
            if (
    ...
    watch([width, height], updateMenuDropLeft, {
      immediate: true,
      flush: "post",
      deep: true,
    });
    ...
    const contentClasses = computed(() => {
      ...
      if (isContentScrollable.value)
        classes.push("horizontal-nav-menu-group-scrollable");

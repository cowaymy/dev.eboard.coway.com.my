...
export default {
	...
	methods: {
		async callSalesHQMainApi() {
			try {
				...
				this.eKeyInData = await salesApi.getEKeyInData(userdata);
				this.netSalesData = await salesApi.getNetSalesData(userdata);
				this.activeData = await salesApi.getActiveData(userdata);
				this.shiData = await salesApi.getSHIData(userdata);
				this.prodcCateory = await salesApi.getProdcCateory(userdata);
				this.tagetSales = await salesApi.getTagetSales(userdata);
			} catch (error) {
				console.log(error);
			} finally {
	...
	data() {
		const testData = this.callSalesHQMainApi2();

		return {
			testData, //call test data
		};
	},
};
</script>

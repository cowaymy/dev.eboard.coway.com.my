  ...
  <div class="h-full">
    <div class="drawer-header d-flex align-center mb-4">
      <span class="font-weight-semibold text-base text--primary"
        >{{ eventLocal.id ? "Update" : "Add" }} Event</span
      >
      <v-spacer></v-spacer>
      <v-btn
        v-if="eventLocal.id"
    ...
    >
      ...
      >
        ...
        <v-btn outlined @click="resetEventLocal"> Reset </v-btn>
      </v-form>
    </perfect-scrollbar>
  </div>
</template>
...
export default {
  ...
  data() {
    ...
    const refCalendarEventHandlerForm = null;

    // ————————————————————————————————————
    //* ——— Local Event
    // ————————————————————————————————————

    const eventLocal = JSON.parse(JSON.stringify(this.event));
    const resetEventLocal = () => {
      eventLocal.value = JSON.parse(JSON.stringify(this.event));

      // Only get date from event
      if (eventLocal.value.start)
        eventLocal.value.start = new Date(eventLocal.value.start)
          .toISOString()
          .substr(0, 10);
      if (eventLocal.value.end)
        eventLocal.value.end = new Date(eventLocal.value.end)
          .toISOString()
          .substr(0, 10);
    ...
    const isStartDateMenuOpen = false;
    const isEndDateMenuOpen = false;

    const guestsOptions = [
      { avatar: require("@/assets/images/avatars/1.png"), name: "Jane Foster" },
      { avatar: require("@/assets/images/avatars/1.png"), name: "Donna Frank" },
      {
        avatar: require("@/assets/images/avatars/1.png"),
        name: "Gabrielle Robertson",
      },
      { avatar: require("@/assets/images/avatars/1.png"), name: "Lori Spears" },
      { avatar: require("@/assets/images/avatars/1.png"), name: "Sandy Vega" },
      { avatar: require("@/assets/images/avatars/1.png"), name: "Cheryl May" },
    ];
    ...
    const endDateValidator = () => {
      ...
      // return (
      // 	!dateInPast(new Date(val), new Date(eventLocal.value.start)) ||
      // 	'End date is in past'
      // );

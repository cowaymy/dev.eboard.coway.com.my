...
let CALENDAR_APP_STORE_MODULE_NAME = 'app-calendar';
let $vuetify = getVuetify();
...
export default {
	...
	data() {
		...
		//* ——— Left Sidebar
		// ————————————————————————————————————
		const { isLeftSidebarOpen, contentStyles } = useResponsiveLeftSidebar({
			sidebarWidth: 250,
		});
			...
			store
				...
				.then(response => {
					...
					events.forEach(fetchedEvent => {
						...
						const eventColor = store.state[
							CALENDAR_APP_STORE_MODULE_NAME
						].calendarOptions.find(
							calendar =>
								calendar.label === fetchedEvent.extendedProps.calendar,
						);

						const rgbColor = hexToRgb(
							$vuetify.theme.currentTheme[eventColor.color],
						);
		...
		const addEvent = eventData => {
			...
			/* eslint-disable no-param-reassign */
			if (eventData.start)
				eventData.start = new Date(eventData.start).toISOString().substr(0, 10);
			if (eventData.end)
				eventData.end = new Date(eventData.end).toISOString().substr(0, 10);

			eventData.timed = false;
			/* eslint-enable */

			store
				.dispatch(`${CALENDAR_APP_STORE_MODULE_NAME}/addEvent`, {
					event: eventData,
				})
				.then(() => {
		...
		const updateEvent = eventData => {
			store
				.dispatch(`${CALENDAR_APP_STORE_MODULE_NAME}/updateEvent`, {
					event: eventData,
				})
				.then(response => {
					const updatedEvent = response.data.event;

					updatedEvent.color = store.state[
						CALENDAR_APP_STORE_MODULE_NAME
					].calendarOptions.find(
						calendar => calendar.label === updatedEvent.extendedProps.calendar,
					).color;

					const updatedEventIndex = calendarEvents.value.findIndex(
		...
		const removeEvent = () => {
			...
			store
				.dispatch(`${CALENDAR_APP_STORE_MODULE_NAME}/removeEvent`, {
					id: eventId,
				})
				.then(() => {
					const updatedEventIndex = calendarEvents.value.findIndex(
		...
		const selectedCalendars = computed(
			() => store.state[CALENDAR_APP_STORE_MODULE_NAME].selectedCalendars,
		);

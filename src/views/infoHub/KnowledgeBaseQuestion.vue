<template>
  <div id="knowledge-base">
    <v-row>
      <v-col cols="12" md="5" lg="3" order="2" order-md="1">
        <v-card v-for="item in kbContentData" :key="item.character">
          <v-card-title class="kbc-title align-center flex-nowrap">
            ...
            <span class="text-truncate"
              >{{ item.title }} ({{ item.questions.length }})</span
            >
          </v-card-title>
      ...
      <v-col cols="12" md="7" lg="9" order="1" order-md="2">
        <v-card>
          ...
          <v-card-text>
            <v-img contain :src="article.postImg"></v-img>
          </v-card-text>

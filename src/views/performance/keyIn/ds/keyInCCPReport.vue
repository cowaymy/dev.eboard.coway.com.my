<template>
  
    <v-card>
        <v-divider ></v-divider>
      <v-card-title class="text--primary mb-0 text-base font-weight-semibold">
        <span>CCP Overview</span>
        <v-spacer></v-spacer>
      </v-card-title>
  
      <v-card-text>
        <v-row>
          <v-col
            cols="12"
            sm="6"
          >
            <apexchart
              id="chart-sales-overview"
              :options="chartOptions"
              :series="chartData"
              class="h-full d-flex align-center"
            ></apexchart>
          </v-col>
  
          <v-col
            cols="12"
            sm="6"
            class="d-flex flex-column justify-center"
          >
            <div class="d-flex align-center">
              <v-avatar
                class="v-avatar-light-bg primary--text"
                rounded
                size="40"
              >
                <v-icon
                  size="30"
                  color="primary"
                >
                  {{ icons.mdiArrowCollapseRight  }}
                </v-icon>
              </v-avatar>
              <div class="ms-4 d-flex flex-column">
                <p class="text--primary mb-0 text-base font-weight-semibold">
                   Active keyIn
                </p>
                <span class="text--primary font-weight-semibold text-xl">{{R_Active}}</span>
              </div>
            </div>
  
            <v-divider class="my-6"></v-divider>
  
            <table class="sales-overview-stats-table">
              <tr>
                <td>
                    <div class="mb-0">
                      <div class="stats-dot d-inline-block rounded-circle me-2"></div>
                      <span class="font-weight-semibold">(IND)</span>
                    </div>
                </td>
                <td>
                  <div class="mb-0">
                    <div class="stats-dot completed d-inline-block rounded-circle me-2"></div>
                    <span>Approved</span>
                  </div>
                  <span class="text-base red--text font-weight-semibold ms-4">{{RCCPApproved}}</span>
                </td>
                <td>
                  <div class="mb-0">
                    <div class="stats-dot active d-inline-block rounded-circle me-2"></div>
                    <span>Active</span>
                  </div>
                  <span class="text-base text--primary font-weight-semibold ms-4">{{RCCPActive}}</span>
                </td>
              </tr>
              <tr>
                <td colspan="4"> <v-divider class="my-6"></v-divider></td>
              </tr>
              <tr>
                <td>
                    <div class="mb-0">
                      <div class="stats-dot d-inline-block rounded-circle me-2"></div>
                      <span class="font-weight-semibold">(COM)</span>
                    </div>
                </td>
                <td>
                  <div class="mb-0">
                    <div class="stats-dot completed d-inline-block rounded-circle me-2"></div>
                    <span>Approved</span>
                  </div>
                  <span class="text-base text--primary font-weight-semibold ms-4">{{OCCPApproved}}</span>
                </td>
                <td>
                  <div class="mb-0">
                    <div class="stats-dot active d-inline-block rounded-circle me-2"></div>
                    <span>Active</span>
                  </div>
                  <span class="text-base text--primary font-weight-semibold ms-4">{{OCCPActive}}</span>
                </td>
              </tr>
            
              <!-- <tr>
                <td>
                  <div class="mb-0">
                    <div class="stats-dot primary d-inline-block rounded-circle me-2"></div>
                    <span>FMCG</span>
                  </div>
                  <span class="text-base text--primary font-weight-semibold ms-4">$30,234</span>
                </td>
                <td>
                  <div class="mb-0">
                    <div class="stats-dot secondary d-inline-block rounded-circle me-2"></div>
                    <span>Other Sales</span>
                  </div>
                  <span class="text-base text--primary font-weight-semibold ms-4">$46,054</span>
                </td>
              </tr> -->
            </table>
          </v-col>
        </v-row>
      </v-card-text>
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn v-if="showList" outlined icon class="me-n3 mt-n1" @click="getKeinListCCP()">
          <v-icon>
            {{ icons.mdiFormatListNumbered  }}
          </v-icon> 
        </v-btn>
      </v-card-actions>
    </v-card>
  </template>
  
  <script>
  import apexchart from "vue-apexcharts";
  import { mdiDotsVertical, mdiCurrencyUsd ,mdiArrowCollapseRight,mdiFormatListNumbered } from '@mdi/js'
  import store from "@/store";

  
  export default {
    components: {
        apexchart
    },
    computed: {

      
        showList(){
          return this.$store.state.userInfo.memberLevel== 0 ? false :true; 
        },


        loading(){
          return this.$store.state.appPerformanceKeyIn.loading
        },

        chartData(){
          return this.$store.state.appPerformanceKeyIn.dataItemCCPList
        },
        
        RCCPActive(){
          return this.fun_numFormat(this.$store.state.appPerformanceKeyIn.dataItemCCPList[0])
        },
        RCCPApproved(){
          //[Individual.Active, Individual.Approved  ,Company.Active,Company.Approved ]*/
          return this.fun_numFormat(this.$store.state.appPerformanceKeyIn.dataItemCCPList[1])
        },
        OCCPApproved(){
          return this.fun_numFormat(this.$store.state.appPerformanceKeyIn.dataItemCCPList[3])
        },

        OCCPActive(){
          return this.fun_numFormat(this.$store.state.appPerformanceKeyIn.dataItemCCPList[2])
        },

        KEYINActive(){
          return this.fun_numFormat(this.$store.state.appPerformanceKeyIn.dataItemCCPList[0] + this.$store.state.appPerformanceKeyIn.dataItemCCPList[1])
        },

        R_Active(){
           let rtnValue =null ;
           const data = this.$store.state.appPerformanceKeyIn.dataItemTotalKeyinList;
           rtnValue=this.fun_numFormat(data['R_ACTIVE'])
           return rtnValue;
        },
    
    },


    methods:{
        fun_numFormat(number) {
          if(number) return  number.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
          //console.log(neFor);
          return null;
        },

        getKeinListCCP(){
            store.dispatch('appPerformanceKeyIn/fetchKeyInOrderListForCCP',{title:" CCP Order List"});
        }
    },
    data() {
      const chartOptions = {
        labels: ['(I)Active', '(I)Approved','(C)Active', '(C)Approved'],
        chart: {
          type: 'donut',
          sparkline: {
            enabled: true,
          },
        },
        plotOptions: {
          pie: {
            donut: {
              size: '70%',
              labels: {
                show: true,
                name: {
                  fontSize: '14px',
                  offsetY: 25,
                },
                value: {
                  fontSize: '2.125rem',
                  fontWeight: 600,
  
                  offsetY: -15,
                  formatter(value) {
                    return `${value}`
                  },
                },
                total: {
                  show: true,
                  label: 'result of CCP',
                  color: 'rgba(94, 86, 105, 0.68)',
                  formatter(value) {
                    return `${value.globals.seriesTotals.reduce((total, num) => total + num)}`
                  },
                },
              },
            },
          },
        },
      }
  
      return {
        chartOptions,
        icons: {
          mdiDotsVertical,
          mdiCurrencyUsd,mdiArrowCollapseRight,mdiFormatListNumbered
        },
      }
    },
  }
  </script>
  
  <style lang="scss">
  #chart-sales-overview {
    .apexcharts-canvas {
      .apexcharts-text {
        &.apexcharts-datalabel-value {
          font-weight: 600;
        }
        &.apexcharts-datalabel-label {
          font-size: 1rem;
        }
      }
    }
  }
  
  .sales-overview-stats-table {
    width: 100%;
    td {
      padding-bottom: 1rem;
    }
  
    .stats-dot {
      padding: 0.33rem;
    }
  
    // Set opacity of dots
    tr {
      &:nth-of-type(1) {
        td:nth-of-type(2) {
          .stats-dot {
            opacity: 0.7;
          }
        }
      }
      &:nth-of-type(2) {
        td:nth-of-type(1) {
          .stats-dot {
            opacity: 0.5;
          }
        }
        td:nth-of-type(2) {
          .stats-dot {
            opacity: 0.15;
          }
        }
      }
    }

    .red--text {
        color: red !important; /* !important를 사용하여 기존 스타일을 덮어씁니다. */
      }

  }
  </style>
  
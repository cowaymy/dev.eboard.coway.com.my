<template>
    <v-card elevation="1">
      <v-card-title class="d-flex align-center pe-2">
        ...
        <span class="font-weight-semibold text-2xl">SHI Hp Ranking by SM </span>
        <v-spacer></v-spacer>
                ...
                >
                  ...
                  <div class="d-flex align-center flex-grow-1 flex-wrap">
                      ...
                      <div class="font-weight-semibold">
                        <span class="text--primary text-xS me-1"
                          >{{ fun_numFormat(data.PCRC) }}<span class="text-xs me-1 lastmonth-font">  ({{ fun_numFormat(data.PMRC) }}) </span> <span class="text-xs me-1 lastyear-font">  ({{ (fun_numFormat(data.PYRC))}}) </span> 
                           </span>  <span class="text-xs me-1">  ({{data.GRP_CODE}})</span>
                        <!-- 
                                  <v-icon
                                      size="20"
                    ...
                    <div>
                      ...
                      <span class="text-xs">({{ data.ORG_CODE  }})</span>
                      <!-- <v-progress-linear
                        height="6"
                        rounded
  ...
  export default {
      ...
      computed: {
        dataList() {
          return  this.$store
                    .state
                    .appPerformanceRcHP.dataItemLevel2List
                    .filter(
                        (item) =>
                      item.NAME
                          .toLowerCase()
      ...
      methods: {
        ...
        gotoLevel3(item){
            this.$store.state.appPerformanceRcHP.dataItemLevel3List  =[];
